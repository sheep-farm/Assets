...
#            print(f"🔍 Zoom: {self.zoom_level * 100:.0f}%")
    ...
    def _on_code_editor_apply(self, node, new_code):
        ...
        node.code = new_code
        self._show_toast(f"✓ Code updated: {node.title}")
        self.queue_draw()
...
class AssetsWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        template_section.append("Save as Template", "win.save-template")
        template_section.append("Load Template", "win.load-template")
        menu.append_section(None, template_section)

        view_section = Gio.Menu()
        view_section.append("Toggle Sidebar", "win.toggle-sidebar")
        view_section.append("Toggle Output", "win.toggle-output")
        menu.append_section(None, view_section)

        # OverlaySplitView (sidebar moderna e colapsável)
        ...
        self.split_view.set_sidebar(self.library_panel)
        ...
        self.content_paned.set_start_child(self.canvas)
        self.content_paned.set_end_child(self.output_panel)
        self.content_paned.set_resize_end_child(False)
        self.content_paned.set_shrink_end_child(False)

        # Estado do output panel
        self.output_panel_visible = True
        self.output_panel_height = 200  # Altura padrão quando visível
        self.content_paned.set_position(500)  # Posição inicial

        # Definir content do split view
        self.split_view.set_content(self.content_paned)

        # Wrap em ToastOverlay
        self.toast_overlay.set_child(self.split_view)

        # Criar HeaderBar com título simples (sem Adw.WindowTitle)
        ...
        # Título usando Label com estilo title (como no Merkato)
        title_label = Gtk.Label()
        title_label.set_label("Assets")
        title_label.add_css_class("title")
        header.set_title_widget(title_label)

        # Use ToolbarView for AdwApplicationWindow
        toolbar_view = Adw.ToolbarView()
        toolbar_view.add_top_bar(header)
        toolbar_view.set_content(self.toast_overlay)

        # AdwApplicationWindow uses set_content, not set_child
...
#        print("✓ Janela criada")
    ...
    def _create_window_actions(self):
        """Cria actions para menu principal"""
        # New
        new_action = Gio.SimpleAction.new("new", None)
        new_action.connect("activate", lambda a, p: self.on_new_clicked(None))
        self.add_action(new_action)

        # Open
        open_action = Gio.SimpleAction.new("open", None)
        open_action.connect("activate", lambda a, p: self.on_open_clicked(None))
        self.add_action(open_action)

        # Save
        save_action = Gio.SimpleAction.new("save", None)
        save_action.connect("activate", lambda a, p: self.on_save_clicked(None))
        self.add_action(save_action)

        # Save As
        save_as_action = Gio.SimpleAction.new("save-as", None)
        save_as_action.connect("activate", lambda a, p: self.on_save_as_clicked(None))
        self.add_action(save_as_action)

        # Save Template
        save_template_action = Gio.SimpleAction.new("save-template", None)
        save_template_action.connect("activate", lambda a, p: self.on_save_template_clicked(None))
        self.add_action(save_template_action)

        # Load Template
        load_template_action = Gio.SimpleAction.new("load-template", None)
        load_template_action.connect("activate", lambda a, p: self.on_load_template_clicked(None))
        self.add_action(load_template_action)

        # Toggle Sidebar (F9)
        toggle_sidebar_action = Gio.SimpleAction.new("toggle-sidebar", None)
        toggle_sidebar_action.connect("activate", lambda a, p: self._toggle_sidebar())
        self.add_action(toggle_sidebar_action)

        # Toggle Output Panel (Ctrl+Shift+O)
        toggle_output_action = Gio.SimpleAction.new("toggle-output", None)
        toggle_output_action.connect("activate", lambda a, p: self._toggle_output_panel())
        self.add_action(toggle_output_action)

        # Keyboard shortcuts
        app = self.get_application()
        if app:
            app.set_accels_for_action("win.toggle-sidebar", ["F9"])
            app.set_accels_for_action("win.toggle-output", ["<Ctrl><Shift>o"])

    def _toggle_sidebar(self):
        """Alterna visibilidade da sidebar (F9)"""
        current = self.split_view.get_show_sidebar()
        self.split_view.set_show_sidebar(not current)
        self.sidebar_toggle.set_active(not current)

    def _toggle_output_panel(self):
        """Alterna visibilidade do painel de output (Ctrl+Shift+O)"""
        if self.output_panel_visible:
            # Esconder: salvar altura atual e colapsar
            self.output_panel_height = self.get_height() - self.content_paned.get_position()
            self.content_paned.set_position(self.get_height() - 1)
            self.output_panel_visible = False
        else:
            # Mostrar: restaurar altura
            new_position = self.get_height() - self.output_panel_height
            self.content_paned.set_position(max(100, new_position))
            self.output_panel_visible = True
    ...
    def _create_library_panel(self):
        ...
        from .node_library import get_all_categories, get_nodes_in_category, get_category_icon, _get_library
        ...
        # Botão de importar biblioteca
        import_btn = Gtk.Button(label="📥")
        import_btn.set_tooltip_text("Import Library")
        import_btn.connect("clicked", self.on_import_library_clicked)
        panel_header.append(import_btn)

        # Botão de exportar biblioteca
        export_btn = Gtk.Button(label="📤")
        export_btn.set_tooltip_text("Export Library")
        export_btn.connect("clicked", self.on_export_library_clicked)
        panel_header.append(export_btn)
        ...
        # Box horizontal para busca + botão
        search_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)

        self.search_entry = Gtk.Entry()
        self.search_entry.set_placeholder_text("Search nodes...")
        ...
        search_button.connect("clicked", lambda b: self.on_search_activate(self.search_entry))
        search_hbox.append(search_button)

        search_box.append(search_hbox)

        # Botão de favoritos
        fav_btn = Gtk.ToggleButton(label="⭐ Favorites")
        fav_btn.connect("toggled", self.on_favorites_toggled)
        search_box.append(fav_btn)
        self.favorites_toggle = fav_btn

        panel_box.append(search_box)
        panel_box.append(Gtk.Separator())
        ...
        # Box para as categorias (armazenar para poder recriar na busca)
        self.categories_scrolled = scrolled
        self.categories_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        self.categories_box.set_margin_top(12)
        self.categories_box.set_margin_bottom(12)
 ...
 #       print(f"✓ Adicionado: {template['name']}")
    ...
    def _populate_node_list(self, nodes=None):
        """Popula lista de nodes no painel lateral"""
        # Bloquear chamadas simultâneas
        ...
        try:
            ...
            # Limpar conteúdo anterior de forma eficiente
            # Coletar todos os children primeiro
            children_to_remove = []
            child = self.categories_box.get_first_child()
            ...
            if nodes is None:
                ...
                for category in get_all_categories():
                    ...
                    self.categories_box.append(category_header)

                    # Nós da categoria
                    category_nodes = get_nodes_in_category(category)
                    for node_template in category_nodes:
                        self._create_node_button(node_template)
            else:
                # Modo busca/filtro: mostrar nodes específicos
                if not nodes:
                    no_results = Gtk.Label(label="No nodes found")
                    no_results.set_margin_top(20)
                    self.categories_box.append(no_results)
                else:
                    for node_template in nodes:
                        self._create_node_button(node_template, show_category=True)
        finally:
            # Sempre desbloquear
            self._is_populating = False

    def _create_node_button(self, node_template, show_category=False):
        """Cria botão para um node template"""
        from .node_library import _get_library

        library = _get_library()

        # Box horizontal para botão + estrela favorito
        ...
        fav_button = Gtk.Button(label="⭐" if is_fav else "☆")
        fav_button.set_has_frame(False)
        fav_button.set_tooltip_text("Toggle favorite")
        fav_button.connect("clicked", self.on_toggle_favorite, node_template["name"])
        hbox.append(fav_button)

        self.categories_box.append(hbox)

    def on_search_activate(self, entry):
        """Callback quando usuário aperta Enter na busca"""
        ...
        # Devolver foco para o search entry
        entry.grab_focus()
    ...
    def _do_search(self, query):
        ...
        # Desativar filtro de favoritos ao buscar
        if query:
    ...
    def _block_signals(self):
        """Context manager para bloquear sinais temporariamente"""
        class SignalBlocker:
    ...
    def on_favorites_toggled(self, button):
        """Callback quando botão de favoritos é ativado"""
        from .node_library import _get_library
    ...
    def on_toggle_favorite(self, button, node_name):
        """Toggle status de favorito de um node"""
        from .node_library import _get_library
        ...
        if self.favorites_toggle.get_active():
            # Se está em modo favoritos, atualizar lista
            favorites = library.get_favorites()
            self._populate_node_list(favorites)
        elif self.search_entry.get_text().strip():
            # Se está buscando, refazer busca
            self.on_search_changed(self.search_entry)
        else:
            # Recriar lista normal
            self._populate_node_list()

    def on_import_library_clicked(self, button):
        """Abre dialog para importar biblioteca"""
        dialog = Gtk.FileDialog()
        dialog.set_title("Import Node Library")

        # Filter para .zip
        filter_zip = Gtk.FileFilter()
        filter_zip.set_name("Library Files (*.zip)")
        ...
        dialog.open(self, None, self._on_import_library_response)

    def _on_import_library_response(self, dialog, result):
        """Callback do dialog de importar"""
        from .node_library import _get_library
        ...
        try:
            ...
            if file:
                ...
                if library.import_library(filepath, merge=True):
                    print("✓ Biblioteca importada com sucesso")
                    self._recreate_library_panel()
        except Exception as e:
            if "dismissed" not in str(e).lower():
                print(f"❌ Erro ao importar: {e}")

    def _update_templates_menu(self, menu_button):
        """Atualiza menu de templates"""
        from .graph_io import get_all_templates

        menu = Gio.Menu()
        templates = get_all_templates()

        if not templates:
            item = Gio.MenuItem.new("No templates available", None)
            menu.append_item(item)
        else:
            for template in templates:
                action_name = f"win.load-template-{template['name'].replace(' ', '-')}"
                item = Gio.MenuItem.new(f"{template['name']} ({template['num_nodes']} nodes)", action_name)
                menu.append_item(item)

                # Criar action para este template
                action = Gio.SimpleAction.new(f"load-template-{template['name'].replace(' ', '-')}", None)
                action.connect("activate", lambda a, p, t=template: self.on_load_template(t))
    ...
    def on_save_template_clicked(self, button):
        """Abre dialog para salvar grafo como template"""
        if not self.canvas.nodes:
            print("⚠️  Grafo vazio, não é possível salvar como template")
            return

        dialog = Gtk.Dialog(title="Save as Template")
        ...
        name_entry.set_placeholder_text("My Template")
        ...
        desc_entry.set_placeholder_text("Description of this template")
        desc_entry.set_activates_default(True)
        content.append(desc_entry)

        dialog.connect("response", self._on_save_template_response, name_entry, desc_entry)
        dialog.present()

    def _on_save_template_response(self, dialog, response, name_entry, desc_entry):
        """Callback do dialog de salvar template"""
        from .graph_io import save_graph_template

        if response == Gtk.ResponseType.OK:
            template_name = name_entry.get_text().strip()
            ...
            if save_graph_template(self.canvas.nodes, self.canvas.connections,
                                  template_name, description):
                print(f"✓ Template salvo: {template_name}")
                # Atualizar menu de templates
                self._update_templates_menu(self.templates_menu_button)

        dialog.destroy()

    def on_load_template(self, template_info):
        """Carrega um template de grafo"""
        from .graph_io import load_graph_template
        from .node import Node

        template_data = load_graph_template(template_info["name"])

        if template_data is None:
            print(f"❌ Erro ao carregar template: {template_info['name']}")
            return
        ...
        connections = []
        for conn_data in template_data.get("connections", []):
        ...
        # Substituir canvas atual
        self.canvas.nodes = nodes
        self.canvas.connections = connections
        self.current_file = None
        self.set_title(f"Assets - {template_info['name']} (from template)")
        self.canvas.queue_draw()

        print(f"✓ Template carregado: {template_info['name']}")
        print(f"  - {len(nodes)} nós")
        print(f"  - {len(connections)} conexões")

    def on_export_library_clicked(self, button):
        """Abre dialog para exportar biblioteca"""
        dialog = Gtk.FileDialog()
        dialog.set_title("Export Node Library")
        dialog.set_initial_name("my_library.zip")
        ...
        dialog.save(self, None, self._on_export_library_response)

    def _on_export_library_response(self, dialog, result):
        """Callback do dialog de exportar"""
        from .node_library import _get_library
        ...
        try:
            ...
            if file:
                ...
                library = _get_library()
                if library.export_library(filepath):
                    print(f"✓ Biblioteca exportada: {filepath}")
        except Exception as e:
            if "dismissed" not in str(e).lower():
    ...
    def on_save_as_clicked(self, button):
        """Wrapper para on_save_as"""
        self.on_save_as()

    def on_load_template_clicked(self, button):
        """Mostra menu de templates disponíveis"""
        from .graph_io import get_all_templates
        templates = get_all_templates()
        if not templates:
            self._show_toast("No templates available")
            return
        # Por enquanto carregar o primeiro template
        # TODO: Mostrar dialog de seleção
        if templates:
            self.on_load_template(templates[0])
    ...
    def _show_toast(self, message, timeout=2):
        """Mostra toast notification (Adwaita style)"""
